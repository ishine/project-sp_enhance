...
void PostFilterInit(){
	...
	for(i=0;i< FRAME_LEN21; i++) { 
		eta_2term[i] = 1;   eta_2term_y[i] = 1;   eta_2term_u[i] = 1; 
	}
}
...
int Dual_Channel_PostFilterProcess(short*pIn, short*pInR, int Inlen, short*pOut, int*Outlen){

	int i,rev ,size; omlsa_float32_t *Sf_ptr, *Conv_I_ptr;
    omlsa_float32_t *Sf,*Sf_u;
    omlsa_float32_t *P_global ;
    omlsa_float32_t *xi_global;
    omlsa_float32_t *P_local ;
    omlsa_float32_t *xi_local ;

	   omlsa_float32_t* Conv_Y_ptr,* Conv_U_ptr;
	   
      omlsa_float32_t gamma , eta, v, gamma_u , eta_u, v_u;				 
  				 
     creal_T Y_COMPLEX[FRAME_LEN21];          creal_T Y_COMPLEXR[FRAME_LEN21];    
	 omlsa_float32_t Y_2[FRAME_LEN21];	
	 omlsa_float32_t U_2[FRAME_LEN21];	
     rev = REV_SUCESS;

// Temp 
     	for(i=0;i< Inlen; i++){	
	    //	pcm[i+FRAME_SHIFT]  = (omlsa_float32_t)pIn[i] ;		 
		//	pcmR[i+FRAME_SHIFT] =  (omlsa_float32_t)pInR[i] ;	

	     	pcm[i]  = (omlsa_float32_t)pIn[i] ;		 
		 	pcmR[i] =  (omlsa_float32_t)pInR[i] ;		

	     }
 

    {  //1.1) WIN & FFT  Beamform Output
    ...
    {  //1.2) WIN & FFT  Reference Noise Output
   
	 omlsa_float32_t*winyR;

	 winyR = om_mem_alloc(FRAME_LEN);
 
     for(i=0;i<FRAME_LEN/2;i++){
     	winyR[i] = pcmR[i]*Window[i];
     } 

    for(i=FRAME_LEN/2;i<FRAME_LEN;i++){
	    winyR[i] = pcmR[i]*Window[FRAME_LEN-i-1];
    }  
	   fft_opt_512(winyR, (omlsa_float32_t*)Y_COMPLEXR );	    
       om_mem_free(FRAME_LEN);
    }

	 U_2[0]   = Y_COMPLEXR[0].re * Y_COMPLEXR[0].re ;
	 U_2[256] = Y_COMPLEXR[0].im * Y_COMPLEXR[0].im ;

	for(i=1;i< FRAME_LEN21-1; i++){	
		U_2[i] = Y_COMPLEXR[i].re * Y_COMPLEXR[i].re +  Y_COMPLEXR[i].im * Y_COMPLEXR[i].im;
	}


	// 2. MCRA noise estimation & voice absent probobality estimate
	 
	// 2.1 Smooth over time 
	/*  if FrameCnt==1 lambda_d=Y_2;  end*/
	if( FrameCnt==1){
		for(i=0; i<FRAME_LEN21; i++) {
			lambda_d_hat[i] = lambda_dav[i] = lambda_d[i] = Y_2[i] ;
            lambda_dav_u[i]   = lambda_d_u[i] = U_2[i] ;  // ref noise
		}
	}
    
      //omlsa_float32_t Sf[FRAME_LEN21+3];
    Sf = om_mem_alloc(FRAME_LEN21+3);  // smooth over freq
	Sf_u = om_mem_alloc(FRAME_LEN21+3);
	b_conv(Y_2, Sf, &size);    b_conv(U_2, Sf_u, &size);
	Sf_ptr = &Sf[1];      
 
	if(FrameCnt==1){
		for(i=0; i<FRAME_LEN21; i++){		  
			Sy[i] = Y_2[i];	 S[i]  = Sf_ptr[i];  St[i] = Sf_ptr[i];
			Su[i] = U_2[i];	 S_u[i]  = St_u[i] = Sf_u[i+1];
		}
	}
	else{
		for(i=0; i<FRAME_LEN21; i++){	
			S[i] = alpha_s * S[i] + (1-alpha_s)*Sf_ptr[i];
			S_u[i] = alpha_s * S_u[i] + (1-alpha_s)*Sf_u[i+1];
		}
	}
	...
	om_mem_free(FRAME_LEN21+3);
     ...
     if(FrameCnt<15) { 
		 for(i=0; i<FRAME_LEN21; i++) {
			 Smin[i] = S[i];       Smact[i] = S[i];
			 Smin_u[i] = S_u[i];   Smact_u[i] = S_u[i];
		 }
	 }
     else{
		 for(i=0; i<FRAME_LEN21; i++)  {
			 Smin[i] = min_local(Smin[i], S[i]);   Smact[i] =  min_local(Smact[i], S[i]); 
			 Smin_u[i] = min_local(Smin_u[i], S_u[i]);   Smact_u[i] =  min_local(Smact_u[i], S_u[i]); 
		 }
	 }
   ...
   
   {
...

	{
	 ...
	 om_mem_free(2*FRAME_LEN21+6); // 2 1  0
 ...
 //  2.3  reference noise 
 {
	...
	for(i=0; i<FRAME_LEN21; i++) {
		if(( U_2[i]<(delta_Y2*Bmin*Smin_u[i])) && (S[i] < delta_s*Bmin*Smin_u[i]) ) {
			I[i]= 1; 
		}
		else {
			I[i]= 0;
...

	{
		...
		omlsa_float32_t*Conv_U= om_mem_alloc( FRAME_LEN21+3); //  2  Conv_Y   2*FRAME_LEN21+6
	    //omlsa_float32_t Y2_I[FRAME_LEN21];
         omlsa_float32_t * U2_I = om_mem_alloc( FRAME_LEN21); // 3 Y2_I    3*FRAME_LEN21+6
     ...
     if(FrameCnt<15){
	     for(i=0;i<FRAME_LEN21;i++){
  		     St_u[i] = S_u[i];  Smint_u[i] = St_u[i];  Smactt_u[i] = St_u[i];
		 }		  
	 }
	 else{
		 for(i=0;i<FRAME_LEN21;i++){
	        ...
	        if( Conv_I_ptr[i])
	            St_u[i]=alpha_s*St_u[i]+(1-alpha_s)*Conv_U_ptr[i]/Conv_I_ptr[i];
		    else 
			    St_u[i]=alpha_s*St_u[i]+(1-alpha_s)*St_u[i];
			 
            Smint_u[i]=min_local(Smint_u[i],St_u[i]);
            Smactt_u[i]=min_local(Smactt_u[i],St_u[i]);
		 }
	 }
 ...
 // 2.4 noise estimate	  
        ...
        xi_frame = 0;
		 ...
		 // Beamform output noise estimate
		for(i=0;i<FRAME_LEN21;i++){ 

			 omlsa_float32_t phat , qhat, Sr_Y2,  Sr_S ,GH1_y  ;
			  
             gamma  = Y_2[i] / max_local(lambda_d[i], 1e-10f); 	    
             eta  = alpha_eta * eta_2term_y[i] + (1-alpha_eta) * max_local(gamma -1,0);
			 ...
			 if(v  > 5){
			     GH1_y  = eta  / (1.0f + eta );
			      ...
			      GH1_y =eta   /(1.0f+eta  ) *exp(0.5f*expint(v  ));       
			 }

			 eta_2term_y[i]=  GH1_y*GH1_y*gamma  ;
        ...
        // Reference noise estimate(lambda_u)
		for(i=0;i<FRAME_LEN21;i++){ 

			 omlsa_float32_t phat_u , qhat_u, Sr_U2,  Sr_S ,alpha_dt_u ,GH1_u ;
			  
             gamma_u  = U_2[i] / max_local(lambda_d_u[i], 1e-10f); 	    
             eta_u  = alpha_eta * eta_2term_u[i] + (1-alpha_eta) * max_local(gamma_u -1,0);
             eta_u  = max_local(eta_u ,eta_min);
             v_u   = gamma_u * eta_u / (1+eta_u );   


			 GH1_u = 1;

			 if(v_u > 5){
			     GH1_u = eta_u / (1.0f + eta_u);
			 }
			 else if(v_u<=5.0f && v_u >0.0f){
			      GH1_u =eta_u  /(1.0f+eta_u ) *exp(0.5f*expint(v_u ));       
			 }

			 eta_2term_u[i]=  GH1_u*GH1_u*gamma_u ;
  
			  Sr_U2  = U_2[i] / (Bmin*max_local(Smint_u[i], 1e-10f)); //Y_2[i] / Bmin / max_local(Smint[i], 1e-10f);
              Sr_S   = S_u[i]/(Bmin*max_local(Smint_u[i],1e-10f)); //S[i]/Bmin/max_local(Smint[i],1e-10f); 
  
			 if(Sr_U2 >1 && Sr_U2 <delta_yt && Sr_S <delta_s){
				  qhat_u= (delta_yt-Sr_U2 )/(delta_yt-1);
				  phat_u =1 /(1+ (qhat_u/(1-qhat_u))*(1+eta_u )*(float)expf(-v_u ));
			 }
			 else if(Sr_U2 >=delta_yt || Sr_S >=delta_s){
				  phat_u  = 1;
			 }
			 else{
				  phat_u  = 0;
             ...
             lambda_dav_u[i] = alpha_dt_u * lambda_dav_u[i] + (1- alpha_dt_u ) * U_2[i];

			//   if(FrameCnt<15){
			// 	  lambda_d_long_u[i]=lambda_dav_u[i];
			//   }
			//   else{
			// 	  alpha_dt_long_u =alpha_d_long +(1-alpha_d_long)*phat_u ;   
            //       lambda_d_long_u[i]=alpha_dt_long_u *lambda_d_long_u[i]+(1-alpha_dt_long_u) *U_2[i]; 
			//   }
		 ...
		 if(FrameCnt==15){
			 for(i=0;i<FRAME_LEN21;i++){
			  ...
			  for(i=0;i<FRAME_LEN21;i++){
				...
				//  SW=[SW(:,2:Nwin) SMact];    Smin=min(SW,[],2);   SMact=S;    
			      ...
			      Smin[i] = min_local(S_his0[i][7], Smin[i]);
			      Smin[i] = min_local(S_his0[i][6], Smin[i]);
                  ...
                  Smint[i] = min_local(S_his[i][7], Smint[i]);
                  Smint[i] = min_local(S_his[i][6], Smint[i]);
                 ...
                 // reference noise 
                  S_his0_u[i][7] = S_his0_u[i][6];
				  S_his0_u[i][6] = S_his0_u[i][5];  
				  S_his0_u[i][5] = S_his0_u[i][4];
  				  S_his0_u[i][4] = S_his0_u[i][3];			 
				  S_his0_u[i][3] = S_his0_u[i][2];
  				  S_his0_u[i][2] = S_his0_u[i][1];
				  S_his0_u[i][1] = S_his0_u[i][0];
				  S_his0_u[i][0] =  Smact_u[i];

			      Smin_u[i] = min_local(S_his0_u[i][7], Smin_u[i]);
			      Smin_u[i] = min_local(S_his0_u[i][6], Smin_u[i]);
                  Smin_u[i] = min_local(S_his0_u[i][5], Smin_u[i]);
  			      Smin_u[i] = min_local(S_his0_u[i][4], Smin_u[i]);					  
  			      Smin_u[i] = min_local(S_his0_u[i][3], Smin_u[i]);
  			      Smin_u[i] = min_local(S_his0_u[i][2], Smin_u[i]);
   			      Smin_u[i] = min_local(S_his0_u[i][1], Smin_u[i]);
                  Smin_u[i] = min_local(S_his0_u[i][0], Smin_u[i]);		  

				  Smact_u[i] = S_u[i];

                  S_his_u[i][7] = S_his_u[i][6];	
                  S_his_u[i][6] = S_his_u[i][5];	
  				  S_his_u[i][5] = S_his_u[i][4];			 
				  S_his_u[i][4] = S_his_u[i][3];
				  S_his_u[i][3] = S_his_u[i][2];
  				  S_his_u[i][2] = S_his_u[i][1];
				  S_his_u[i][1] = S_his_u[i][0];
				  S_his_u[i][0] =  Smactt_u[i];

                  Smint_u[i] = min_local(S_his_u[i][7], Smint_u[i]);
                  Smint_u[i] = min_local(S_his_u[i][6], Smint_u[i]);
                  Smint_u[i] = min_local(S_his_u[i][5], Smint_u[i]);
				  Smint_u[i] = min_local(S_his_u[i][4], Smint_u[i]);
				  Smint_u[i] = min_local(S_his_u[i][3], Smint_u[i]);
  				  Smint_u[i] = min_local(S_his_u[i][2], Smint_u[i]);
   			      Smint_u[i] = min_local(S_his_u[i][1], Smint_u[i]);
                  Smint_u[i] = min_local(S_his_u[i][0], Smint_u[i]);
     ...
     for(i=0;i<FRAME_LEN21;i++){
		 ...
		 lambda_d_u[i] = 1.4685f*lambda_dav_u[i];		 
	 }

     // 2.5 speech absent probobilty 
     {
		 double LAMBDA_Y  , LAMBDA_U ,OMEGA,gamma_s ;
         double OMEGA_HI = 3.0f, OMEGA_LO = 1.0f, gamma_0 = 4.6f, PSI_0 = 0.25f; 

		 double k1_psi = 9; double k2_psi = 113; 
		  PSI_global = 0;
          for(  i = 0; i < FRAME_LEN21; i++)
		  {
			   LAMBDA_Y = S[i] / max_local(lambda_d[i],1e-10f);
   			   LAMBDA_U = S_u[i] / max_local(lambda_d_u[i],1e-10f);

				PSI[i] = 0.0f;

			   if((LAMBDA_Y  > LAMDBA_0) && (LAMBDA_U  < LAMDBA_0)){	
			        PSI[i] = 1.0f;
				 }
				else if((LAMBDA_Y  > LAMDBA_0) && (LAMBDA_U  > LAMDBA_0)){
                    OMEGA = (S[i] - lambda_d[i])/max_local((S_u[i]-lambda_d_u[i]),1e-10f);
					...
					{
						PSI[i] = (OMEGA - OMEGA_LO) / (OMEGA_HI - OMEGA_LO);
					}					
				}	

                if(i>=k1_psi && i<=k2_psi){
					PSI_global += PSI[i];
				}
		  }
		  
           PSI_global/=(k2_psi - k1_psi+1);
		  
		  for(  i = 0; i < FRAME_LEN21; i++){
			  gamma_s = Y_2[i]/max_local(lambda_d[i],1e-10f);
              gamma_s = min_local(gamma_s, gamma_0); 

			  if(gamma_s > 1 && PSI_global >PSI_0)  {
				  q_hat[i] = max_local((gamma_0-gamma_s) / (gamma_0-1) , 1- PSI[i]);
			  }
			  else{
				  q_hat[i]  = 1.0f;
			  }
	...
	// omlsa_float32_t xi_global[FRAME_LEN21+32];
	  ...
	  if(xi_frame>0){
       ...
       if( xi_frame_dB<=xi_min_dB){
        ...
        for(i=0;i<FRAME_LEN21;i++){	
			omlsa_float32_t q, PH1, GH0,GH1,G, alpha_d_hat;
 	        omlsa_float32_t minval,lambda_d_global;	
           q = 1 - P_frame*P_global[i]*P_local[i];
         //   q = min_local(qmax,q);

		  q = min_local(qmax,q_hat[i]); // use q_hat instead 

           if((i<=2)||(i>=FRAME_LEN21-3)){
                lambda_d_global =  lambda_d_hat[i];
			}
  
           minval = min_local(lambda_d_hat[i] , lambda_d_hat[i-3] );
           lambda_d_global  = min_local(minval , lambda_d_hat[i+3] );		   
 
           gamma  = Y_2[i] / max_local(lambda_d_hat[i], 1e-10f);
           ...
           if(q<0.9f){
		    ...
		    lambda_d_hat[i] = alpha_d_hat  * lambda_d_hat[i] + (1-alpha_d_hat ) *Y_2[i];
