                                   ...
                                   int16_t nrSamples) // (i) number of samples
{
    ...
    for (subfr = 0; subfr < BLK_NUM; subfr++) // 10
    {
        // downsample to 4 kHz
        if (nrSamples == 160)
        {
            for (k = 0; k < 8; k++) // 16 ->   c = (a + b) /2
            {
                tmp32 = (int32_t)in[2 * k] + (int32_t)in[2 * k + 1]; //16000->8000 hz, c = (a+b)/2
                tmp32 = WEBRTC_SPL_RSHIFT_W32(tmp32, 1);
                buf1[k] = (int16_t)tmp32;
            }
            in += 16;

            WebRtcSpl_DownsampleBy2(buf1, 8, buf2, state->downState); // 8 -> 4
