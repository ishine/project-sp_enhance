                                 ...
                                 int16_t lowlevelSignal)
{
   ...
   if (FS == 16000)
    ...
    if (in_near != out)
    {
        ...
        memcpy(out, in_near, 10 * L * sizeof(int16_t)); // 10 * L = 160
    }

    // VAD for near end
    logratio = WebRtcAgc_ProcessVad(&stt->vadNearend, out, L * 10);// 下采样到4khz采样率，高通滤波计算值，并用log方法计算db
