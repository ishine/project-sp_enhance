...
#define BLK_NUM 8 // 10 * 16 = 160 sample 
                                 ...
                                 int16_t lowlevelSignal)
{
   ...
   if (FS == 16000)
    ...
    if (in_near != out)
    {
        ...
        memcpy(out, in_near, BLK_NUM * L * sizeof(int16_t)); // 10 * L = 160
    }

    // VAD for near end
    logratio = WebRtcAgc_ProcessVad(&stt->vadNearend, out, L * BLK_NUM);// 下采样到4khz采样率，高通滤波计算值，并用log方法计算db
    ...
    if (gate > 0)
    {
        if (gate < 2500)
        {
            gain_adj = WEBRTC_SPL_RSHIFT_W16(2500 - gate, 5); // (2500 - gate) >> 5
        } else
        {
            gain_adj = 0;
        }
...
void WebRtcAgc_InitVad(AgcVad_t *state)
{
 ...
 //	    state->varianceLongTerm = WEBRTC_SPL_LSHIFT_W32(2000, 8);
    ...
    state->varianceShortTerm = WEBRTC_SPL_LSHIFT_W32(500, 8);
	 //state->varianceShortTerm = WEBRTC_SPL_LSHIFT_W32(2000, 8);
                                   ...
                                   int16_t nrSamples) // (i) number of samples
{
    ...
    for (subfr = 0; subfr < BLK_NUM; subfr++) // 10 -> 8  160 -> 128
    {
        // downsample to 4 kHz
        if (nrSamples == 128)
        {
            for (k = 0; k < 8; k++) //   c = (a[i] + a[i+1])/2
            {
                tmp32 = (int32_t)in[2 * k] + (int32_t)in[2 * k + 1]; //16000->8000 hz buf1
                tmp32 = WEBRTC_SPL_RSHIFT_W32(tmp32, 1);
                buf1[k] = (int16_t)tmp32;
            }
            in += 16;

            WebRtcSpl_DownsampleBy2(buf1, 8, buf2, state->downState); //  8000hz->4000hz buf2
        ...
        for (k = 0; k < 4; k++)
        {
            ...
            tmp32 = WEBRTC_SPL_MUL(out, out);  // out*out
            nrg += WEBRTC_SPL_RSHIFT_W32(tmp32, 6);
        }
    }
    state->HPstate = HPstate;

    // find number of leading zeros  must be positive value, so check the bit number
    if (!(0xFFFF0000 & nrg)) // nrg <= 16bit
    {
        zeros = 16;  
    } else
    {
    ...
    if (!(0xFF000000 & (nrg << zeros))) //nrg <= 0x1000000
    {
        zeros += 8;
    }
