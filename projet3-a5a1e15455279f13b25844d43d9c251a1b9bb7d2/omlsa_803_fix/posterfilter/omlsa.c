...
creal_T Y_COMPLEX2[FFT_LEN];   
creal_T Y_out[FFT_LEN];  
creal_T Y_out2[FFT_LEN];  

omlsa_float32_t Y_2[FRAME_LEN21];	
    
omlsa_float32_t winy_test[FFT_LEN*2];
creal_T Y_COMPLEX_test[FFT_LEN];    	 
omlsa_float32_t  winy2[FFT_LEN];  //  A
omlsa_float32_t  winy2out[FFT_LEN];  //  A
...
int PostFilterProcess(short*pIn, int Inlen, short*pOut, int*Outlen){
     ...
     rev = REV_SUCESS;
      ...
      gpio_on1();
	   ...
	   for(i=0;i<FRAME_LEN;i++){
		  winy_test[i] = (omlsa_float32_t)pcm[i]*Window[i];// / 32768.0f;
		  winy2[i] = (omlsa_float32_t)pcm[i]*Window[i];// / 32768.0f;
		 
		  winy_test[i+FRAME_LEN] = 0; 
		  winy2[i+FRAME_LEN] = 0; 
	   } 
...
#if  fft_512_opt
	   fft_opt_512(winy2, (omlsa_float32_t*)Y_COMPLEX );

	//   Y_COMPLEX[0].re = winy2out[0];
	//   Y_COMPLEX[0].im = 0;
	//
	//   Y_COMPLEX[256].re = winy2out[1];
	//   Y_COMPLEX[256].im = 0;
	//
	//   for(i=1;i< 256;i++){	   
	//	   Y_COMPLEX2[i].re = winy2out[2*i];
	//	   Y_COMPLEX2[i].im = winy2out[2*i+1];
	//   }
	    
	//   ifft_opt_512((omlsa_float32_t*)winy2out,(omlsa_float32_t*)winy2 );

	
#else 
	 fft(winy_test,Y_COMPLEX);	     
#endif
 ...
 //0.138
  ...
  //46us
	 ...
	 om_mem_free(FRAME_LEN21*4 + 6);  // free 3 2
...
#if fft_512_opt   
	        ifft_opt_512((omlsa_float32_t*)Y_COMPLEX,(omlsa_float32_t*)winy2out );

			  for(i=0;i<512;i++){			  
			     x_g_c[i].re = winy2out[i];
			  }
#else 
