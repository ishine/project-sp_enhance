	...
	int i,rev ,size; omlsa_float32_t *Sf_ptr, *Conv_I_ptr;
     ...
     rev = REV_SUCESS;
      ...
      gpio_on1();
	   ...
	   {
		   omlsa_float32_t *pWIn;
 

        pWIn = Window;
	   for(i=0;i<FRAME_LEN/2;i++){		  
		  winy[i] = (omlsa_float32_t)pcm[i]*pWIn[i]; 	  
	 
	   } 

	   pWIn = &Window[FRAME_LEN/2-1];

	   for(i=FRAME_LEN/2;i<FRAME_LEN ;i++){		  
		  winy[i] = (omlsa_float32_t)pcm[i]* *pWIn--;   
...
//0.176ms    
 ...
 gpio_off2();
 ...
 
        {
		    ...
		    x_g_c = ( omlsa_float32_t *)om_mem_alloc(FFT_LEN);
	   gpio_on2();//0.622ms
 ...
 
{
            ...
            short *p2;
            int temp;
			p0 = (omlsa_float32_t*) x_g_c;//&x_g_c[0].re;
            p1 = &Window[0];
			p2 = out_buf;
