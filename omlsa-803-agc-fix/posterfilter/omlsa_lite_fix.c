...
void b_conv_fix(const omlsa_float32_t A_data[], omlsa_float32_t C_data[], int C_size[1]);

float Sf1[257], Sf2[257];
...
int PostFilterProcess(short*pIn, int Inlen, short*pOut, int*Outlen){
    ...
    omlsa_float32_t gamma , eta, v;			 
	 ...
	 creal_T_fix   Y_COMPLEX_Fix[FRAME_LEN21];    
...
// Temp
   ...
   //omlsa_float32_t Sf[FRAME_LEN21+3];
	...
	if(FrameCnt==1){
		...
		for(i=0; i<FRAME_LEN21; i++){	
			S[i] = alpha_s * S[i] + (1-alpha_s)*Sf_ptr[i];//q4 alpha_s
		}
	}
...
for(i=0;i<FRAME_LEN21;i++){
	  ...
	  if(xi_frame>0){
       ...
       if( xi_frame_dB<=xi_min_dB){
	     ...
	     // I:  P_frame P_global  P_local  [0~1]
         // O:  q [0~0.998f]
        for(i=0;i<FRAME_LEN21;i++){	
			omlsa_float32_t q, PH1, GH0,GH1,G;
            int q_fix, PH1_fix, GH0_fix,GH1_fix,G_fix;

 	        omlsa_float32_t minval,lambda_d_global;	
            int minval_fix,lambda_d_global_fix;	
           ...
           if(v>5){
  			...
  			if(i<3 ||i==(FRAME_LEN21-1)){
			   Y_COMPLEX_Fix[i].re = 0;
  			   Y_COMPLEX_Fix[i].im = 0;
			   Y_COMPLEX[i].re = 0;
  			   Y_COMPLEX[i].im = 0;
			}
			else {
			   ...
			   G_fix = G * Q24;        // Q24
               Y_COMPLEX_Fix[i].re = Y_COMPLEX[i].re * Q16; //Q16
			   temp = (long long)Y_COMPLEX_Fix[i].re * (long long)G_fix;  
			   Y_COMPLEX_Fix[i].re = (int)(temp>> 24) ;
 
               Y_COMPLEX_Fix[i].im = Y_COMPLEX[i].im * Q16;
			   temp = (long long)Y_COMPLEX_Fix[i].im * (long long)G_fix;   
               Y_COMPLEX_Fix[i].im = (int)(temp>> 24) ;

 			   Y_COMPLEX[i].re = (float)Y_COMPLEX_Fix[i].re  / Q16;
			   Y_COMPLEX[i].im = (float)Y_COMPLEX_Fix[i].im  / Q16;			
			}          
    ...
    //  Y_COMPLEX[256].im = 0.0f;
		 
        {
		    ...
		    for(i=0;i<FRAME_LEN/2;i++){	 
                 int temp;
#if FIX_VERSION
                 temp   =  x_g_c[i]  *Window[i]* Cwin   ;
 	             x_g_c_fix[i] = (temp>32767)? 32767 : ((temp<-32768) ? -32768: (short)temp  );
...
#if FIX_VERSION
				int temp;
				temp =  x_g_c[i]  *Window[FRAME_LEN-i-1]* Cwin   ;
 	             x_g_c_fix[i] = (temp>32767)? 32767 : ((temp<-32768) ? -32768: (short)temp  );
#else 
		    ...
		    for(i=0;i<FRAME_LEN;i++){ 
				...
				temp = (int)out_buf[i] + (int)x_g_c_fix[i] ;  //temp*=32768.0f;
#else 
				float temp;
